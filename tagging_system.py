...
class DocumentPage:
    ...
    def extract(self, segmentation_model: SegmentationModel, finding_formulas_model: FindingFormulasModel, langs: list, tessdata_dir: str):
        ...
        # Disable math formulares extract
...
class Document:
    ...
    def build_docx_documnet(self, output_filename:str, only_text = False):
        ...
        for i in range(num_pages):
            ...
            for j in range(num_elements):
                ...
                elif page.elements[j].element_type == 'figure' and not only_text:
                    document.add_picture(page.elements[j].image_path, width=Inches(5))
                elif page.elements[j].element_type == 'table' and not only_text:
    ...
    def convert(self, output_type: str, output_filename:str, to_zip: bool) -> str:
        ...
        elif output_type == 'docx_only':
            self.build_docx_documnet(output_filename, only_text=True)
            return f'./output/{self.output_dir}/{output_filename}'
        elif output_type == 'only_figure':
            path_images = os.listdir(f'./output/{self.output_dir}/images/')
            for i in range(len(path_images)):
                path_images[i] = f'./output/{self.output_dir}/images/' + path_images[i]
            return path_images
